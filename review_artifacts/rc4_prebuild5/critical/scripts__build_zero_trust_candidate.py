...
def main() -> int:
    ...
    runtime_dir = args.l3_runtime.resolve()
    assert_source_test_runtime_capability(extra_runtime=runtime_dir)

    with tempfile.TemporaryDirectory(prefix="imagelab-build-snapshot-") as tmp:
        temp_root = Path(tmp)
        build_root = temp_root / "source"
        snapshot_tree = copy_source_snapshot(ROOT, build_root)
        source_gate_receipt = temp_root / "source-gate-receipt.json"
        gate = run_full_source_gate(
            root=build_root,
            runtime_dir=runtime_dir,
            receipt_path=source_gate_receipt,
            expected_source_tree=snapshot_tree,
        )
        approved_source_tree = dict(gate["source_tree"])
        approved_runtime_tree = dict(gate["runtime_tree"])
        if approved_source_tree != snapshot_tree:
            raise RuntimeError(
                f"source gate approved a tree different from the immutable snapshot: "
                f"snapshot={snapshot_tree}, approved={approved_source_tree}"
            )
        if compute_source_tree(build_root) != snapshot_tree:
            raise RuntimeError("tested source snapshot changed after source gate")

        output_dir = args.output_dir.resolve()
        output_parent = output_dir.parent
        output_parent.mkdir(parents=True, exist_ok=True)
        safe_label = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in args.label)
        final_installer_path = output_dir / f"ImageLab_by_LarannA_{safe_label}_Setup_x64.exe"
        final_source_zip = output_dir / f"ImageLab_by_LarannA_{safe_label}_SOURCE.zip"
        final_manifest_path = output_dir / "candidate-manifest.json"
        final_sha_path = output_dir / "installer-sha256.txt"
        ...
        try:
            with tempfile.TemporaryDirectory(prefix=".imagelab-private-candidate-", dir=output_parent) as private_tmp:
                ...
                try:
                    ...
                    pass
                installer_path = private_stage / final_installer_path.name
                source_zip = private_stage / final_source_zip.name
                manifest_path = private_stage / final_manifest_path.name
                sha_path = private_stage / final_sha_path.name
                ...
                installer_sha = sha256(installer_path)
                ...
                candidate = {
                    ...
                    "identity": identity,
                    "source_gate": gate,
                    "installer": {"path": str(final_installer_path), "name": final_installer_path.name, "sha256": installer_sha, "size_bytes": installer_path.stat().st_size, "pe": pe},
                    "payload": {"path": str(payload_path), "sha256": sha256(payload_path), "entries": len(payload_names), "crc_ok": bad_payload is None, "missing_required": missing},
                    "source": {"path": str(final_source_zip), "sha256": source_sha, "entries": source_count, "crc_ok": bad_source is None, "tested_snapshot_tree": approved_source_tree},
                }
                ...
                sha_path.write_text(f"{installer_sha}  {final_installer_path.name}\n", "utf-8")

                output_dir.mkdir(parents=True, exist_ok=True)
                os.replace(installer_path, final_installer_path)
                os.replace(source_zip, final_source_zip)
                os.replace(manifest_path, final_manifest_path)
                os.replace(sha_path, final_sha_path)
