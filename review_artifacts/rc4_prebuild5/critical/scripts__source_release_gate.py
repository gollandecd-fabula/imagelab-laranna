...
COLLECTION_MANIFEST_SHA256 = "766043a9e1516ae80fefd61f2af37354c35525e9908d4ec0f301890c0f9c9e38"
...
def _source_file(root: Path, path: Path) -> bool:
    ...
    if any(part in EXCLUDED_PARTS for part in rel.parts):
        return False
    if path.suffix in {".pyc", ".pyo"} or path.name in {".coverage", "verified-runtime-receipt.json"}:
        return False
    ...
    if rel.parts and rel.parts[0].startswith("evidence"):
        return False
    if path.suffix == ".exe" and rel != Path("vendor/tools/stet/stet.exe"):
        return False
    return path.is_file()
...
def iter_source_files(root: Path = ROOT) -> list[Path]:
    root = root.resolve()
    return [path for path in root.rglob("*") if _source_file(root, path)]
...
def _runtime_tree(runtime_dir: Path) -> dict[str, object]:
    try:
        ...
        from scripts.l3_dependency_intake import compute_runtime_tree  # type: ignore
...
def build_pytest_environment(*, root: Path, runtime_dir: Path, base_env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for key in tuple(env):
        upper = key.upper()
        if upper.startswith("PYTHON") or upper.startswith("PYTEST"):
            env.pop(key, None)
...
def _trusted_interpreter_site_paths() -> tuple[str, ...]:
    ...
    for value in candidates:
        ...
        try:
            ...
            raise RuntimeError(f"interpreter library path escapes sys.prefix: prefix={prefix}, path={resolved}") from exc
        text = str(resolved)
        if text not in trusted:
            trusted.append(text)
    if not trusted:
        raise RuntimeError("no trusted interpreter site-packages path available for isolated pytest")
...
def _isolated_pytest_bootstrap(*, root: Path, runtime_dir: Path) -> str:
    ...
    return (
        "import sys; "
        f"trusted=[{runtime!r},{source!r},*{interpreter_paths!r}]; "
        "clean=[]; "
        "[clean.append(p) for p in trusted if p and p not in clean]; "
        "sys.path[:0]=clean; "
        "import pytest; "
...
def _manifest_path(root: Path) -> Path:
    return root.resolve() / COLLECTION_MANIFEST_REL
...
def collect_test_nodeids(*, root: Path, runtime_dir: Path) -> tuple[str, ...]:
    env = build_pytest_environment(root=root, runtime_dir=runtime_dir)
    completed = subprocess.run(build_collect_command(root=root,runtime_dir=runtime_dir),cwd=root,env=env,text=True,capture_output=True)
...
def verify_complete_collection(*, root: Path, runtime_dir: Path) -> tuple[str, ...]:
    ...
    if actual != expected:
        missing=[node for node in expected if node not in set(actual)]; extra=[node for node in actual if node not in set(expected)]
        raise RuntimeError("pytest collection differs from pinned manifest: "+json.dumps({"expected":len(expected),"actual":len(actual),"missing":missing[:20],"extra":extra[:20]},sort_keys=True))
    return expected
...
def write_pass_receipt_for_test(*, root: Path, runtime_dir: Path, receipt_path: Path, tests_total: int, tests_passed: int) -> dict[str, object]:
    ...
    receipt={"schema":"imagelab.source-release-gate.v5","status":"PASS","execution_policy":dict(EXECUTION_POLICY),"source_tree":source,"runtime_tree":runtime,"tests":{"total":tests_total,"passed":tests_passed,"failures":0,"errors":0,"skipped":0}}
    receipt_path.write_text(json.dumps(receipt,ensure_ascii=False,indent=2),"utf-8"); return receipt
...
def run_full_source_gate(*, root: Path, runtime_dir: Path, receipt_path: Path, expected_source_tree: dict[str, object] | None = None) -> dict[str, object]:
    assert_runtime_outside_source(root=root,runtime_dir=runtime_dir)
    source_before=compute_source_tree(root); expected_source_tree=dict(expected_source_tree or source_before)
    ...
    except ImportError: from scripts.l3_dependency_intake import validate_verified_runtime  # type: ignore
    validate_verified_runtime(runtime_dir=runtime_dir)
    _assert_expected_source_tree(root=root,expected=expected_source_tree,phase="after-runtime-validation")
    try: from release_capability_preflight import assert_source_test_runtime_capability
    except ImportError: from scripts.release_capability_preflight import assert_source_test_runtime_capability  # type: ignore
    assert_source_test_runtime_capability(extra_runtime=runtime_dir)
    _assert_expected_source_tree(root=root,expected=expected_source_tree,phase="after-capability-preflight")
    expected_nodeids=verify_complete_collection(root=root,runtime_dir=runtime_dir)
    _assert_expected_source_tree(root=root,expected=expected_source_tree,phase="after-collection")
    env=build_pytest_environment(root=root,runtime_dir=runtime_dir)
    with tempfile.TemporaryDirectory(prefix="imagelab-source-gate-") as tmp:
        junit=Path(tmp)/"pytest.xml"; command=build_pytest_command(junit,root=root,runtime_dir=runtime_dir)
        completed=subprocess.run(command,cwd=root,env=env,text=True,capture_output=True)
        source_after_pytest=_assert_expected_source_tree(root=root,expected=expected_source_tree,phase="after-pytest")
    ...
    if completed.returncode!=0 or tests["failures"] or tests["errors"] or tests["skipped"] or tests["tests"]!=expected_total or tests["passed"]!=expected_total:
        raise RuntimeError("full source gate failed: "+json.dumps({"returncode":completed.returncode,"expected_total":expected_total,**tests},sort_keys=True))
    runtime_tree=_runtime_tree(runtime_dir)
    _assert_expected_source_tree(root=root,expected=expected_source_tree,phase="receipt-write")
    receipt={"schema":"imagelab.source-release-gate.v5","status":"PASS","execution_policy":dict(EXECUTION_POLICY),"collection":{"manifest_sha256":COLLECTION_MANIFEST_SHA256,"node_count":expected_total},"source_tree":source_after_pytest,"runtime_tree":runtime_tree,"tests":{"total":expected_total,"passed":expected_total,"failures":0,"errors":0,"skipped":0},"pytest":{"command":command}}
    receipt_path.parent.mkdir(parents=True,exist_ok=True); receipt_path.write_text(json.dumps(receipt,ensure_ascii=False,indent=2),"utf-8"); return receipt
