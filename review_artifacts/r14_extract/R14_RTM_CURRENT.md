...
| R14-7 | F03->F04 | Extract, Improve, Remove Background remain distinct | preserve pipeline separation | real API chain | compatibility L2 | composite E2E | F03 preserves data; F04 removes bg | PARTIAL — real API chain PASS on fresh compatibility scaffold |
...
| R14-9 | Client | Manual UI routes strict region working-fragment path | split region vs auto request parameters | exact snippet + Node routing harness | L1 independently reproduced | R14_APP_EXTRACT_ROUTING.patch + R14_CLIENT_ROUTING_EVIDENCE.md + R14_REPRO_MANIFEST.json | region sends ROI/strict/perspective only; auto cutout controls unchanged | PARTIAL — committed routing is independently reproducible; exact full product client provenance unavailable |
| R14-10 | Regression | Focused + neighboring regressions | deterministic split regression sets + clean-checkout runner | custom/focused/broad/browser/repro | L1 independent + compatibility L2 | exact extraction evidence + R14_REPRO_MANIFEST.json + composite rehearsal evidence | no R14 regression; reproducible committed checks | PARTIAL — Codex clean checkout 8/8 PASS and fresh composite tests PASS; 3 superseded old-contract assertions and missing pinned dependency classes remain explicitly separated |
| R14-11 | Packaging | R14 exact packaged runtime | build/package exact verified source | source-tree/apply gate + dependency intake + packaged E2E | compatibility source rehearsal + blocker evidence | R14_APPLY_COMPOSITE_TO_PREBUILD6.py + R14_COMPOSITE_REHEARSAL_EVIDENCE.md | R14 code executes in exact package | BLOCKED — fresh 215-file rehearsal tree SHA b992edd8… verified with exactly 4 changed files, but exact full R13 tree and pinned verified runtime wheel bytes remain unavailable; intake exits 2 fail-closed |
...
| R14-14 | QA boundary | Working fragment must not be rejected/repaired as cutout | semantic QA + manual repair guard | direct QA + FastAPI + auto regression | L1 + compatibility L2 | R14_QA_REPAIR_SEMANTIC_ROUTING.patch + QA evidence + fresh composite rehearsal | correct working fragment accepted; bad contract blocked; auto unchanged | PARTIAL — fresh compatibility QA 5/5 PASS; exact R13 QA/repair provenance unavailable |
