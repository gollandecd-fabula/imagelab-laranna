...
def test_missing_project_rename_and_preset_are_fail_closed() -> None:
    ...
    rename = client.patch(f"/api/projects/{project_id}/title", json={"title": "Не создавать"})
    preset = client.put(f"/api/projects/{project_id}/presets", json={"name": "X", "module": "cleanup", "parameters": {}})
...
def test_check_only_diverts_every_frontend_mutation() -> None:
    ...
    for function_name in ("processSelected", "applyCleanupFlow", "exportSelected"):
...
def test_ai_autoparams_cannot_overwrite_explicit_halftone_values() -> None:
    source = (ROOT / "app" / "services" / "image_processing.py").read_text("utf-8")
    halftone = source[source.index('if normalized == "halftone"'):source.index('if normalized == "vectorize"')]
    for key in ("raster", "size_mm", "density"):
        assert f'if not _provided(params.get("{key}"))' in halftone


def test_exact_twenty_six_file_release_matrix_is_fail_closed() -> None:
