...
def _asset(asset_id: str, name: str, digest_char: str) -> dict:
    return {
        "id": asset_id,
        "original_name": name,
...
def _load(page):
    ...
    project = {
        ...
        "updated_at": "x",
        "assets": assets,
        "workspace": {"active_asset_id": "aaaaaaaa", "active_revision": 1, "presets": {}},
    }
    images = {
        "aaaaaaaa": _png((220, 70, 70)),
        "bbbbbbbb": _png((50, 100, 220)),
        "cccccccc": _png((40, 180, 100)),
    }
    ...
    def handler(route):
        ...
        if path == "/api/projects":
            ...
            body = project
        elif path == "/api/projects/TS-001/active":
            project["workspace"]["active_asset_id"] = request.post_data_json["asset_id"]
            project["workspace"]["active_revision"] += 1
            body = project
        elif path.startswith("/api/m2a/projects/TS-001/workspace/") and request.method == "PUT":
            section = path.rsplit("/", 1)[-1]
            project["workspace"][section] = request.post_data_json["value"]
            body = {"project": project, "section": section, "status": "saved"}
        elif path == "/api/m2a/projects/TS-001/presets" and request.method == "PUT":
            ...
            time.sleep(0.06)
            payload = request.post_data_json
            process_calls.append(payload["asset_id"])
            source = next(asset for asset in project["assets"] if asset["id"] == payload["asset_id"])
            result = dict(source)
            result.update(
                {
                    "id": f"result{len(process_calls):02d}",
                    "original_name": f"result-{len(process_calls)}.png",
                    "source_asset_id": source["id"],
                    "operation": payload["operation"],
                    "sha256": str(len(process_calls)) * 64,
                    "preview_url": f"/preview/{source['id']}.png",
                }
            )
            ...
            key = Path(path).stem
            route.fulfill(status=200, content_type="image/png", body=images.get(key, images["aaaaaaaa"]))
            return
        else:
            body = {
                "project": project,
    ...
    expect(page.locator("#projectName")).to_have_text("M2A Matrix")
    return project, process_calls


def _wait_until(page, predicate, *, timeout_ms: int = 5000) -> None:
...
def _alpha_sum(page) -> int:
    return int(
        page.locator("canvas.m2a-selection-layer").evaluate(
            """canvas => {
...
def test_m2a_visual_matrix_navigation_and_scope_lock(browser) -> None:
    ...
    for width in (800, 1024, 1280, 1440, 1920):
        ...
        expect(page.locator('[data-pane="geometry"]')).to_have_class("module-pane active")
        assert page.evaluate("document.documentElement.scrollWidth - window.innerWidth") <= 1
        page.screenshot(path=str(ARTIFACT_DIR / f"m2a-{width}.png"), full_page=True)
        page.close()

    page = browser.new_page(viewport={"width": 1280, "height": 900})
    _load(page)
    assert page.locator(".nav-svg").count() == page.locator(".nav-item").count()
    modules = (
    ...
    for action in (
        "m2aApplyBackground", "m2aApplyColor", "m2aDtfQa",
        "m2aDtfMaster", "m2aCardMaster", "m2aPrepareLogo",
    ):
        expect(page.locator(f"#{action}")).to_be_disabled()
    expect(page.locator("#m2aDtfStatus")).to_contain_text("BLOCKED")
    page.locator("#m2aPreparePrint").click()
    expect(page.locator("#m2aAutopilotDialog")).to_have_class("m2a-autopilot-dialog open")
...
def test_m2a_size_preview_mask_and_workspace_persistence(browser) -> None:
    ...
    prior_height = page.locator("#heightMm").input_value()
    page.locator("#widthMm").fill("120")
    expect(page.locator("#heightMm")).to_have_value(prior_height)
    ...
    assert page.locator("#m2aZoomValue").inner_text().endswith("%")

    before_assets = len(project["assets"])
    page.locator('[data-preview-only="geometry"]').click()
    expect(page.locator(".m2a-preview-note").first).to_contain_text("новая версия не создана")
    assert len(project["assets"]) == before_assets

    page.locator("#usePerspective").check()
    expect(page.locator(".m2a-perspective-handle")).to_have_count(4)
    handle = page.locator(".m2a-perspective-handle").first
    ...
    page.mouse.down()
    page.mouse.move(start_x + 50, start_y + 50, steps=8)
    page.mouse.up()
    assert float(page.locator("#pTLx").input_value()) > 0
    ...
    assert canvas_box
    page.mouse.move(
        canvas_box["x"] + canvas_box["width"] * 0.2,
        canvas_box["y"] + canvas_box["height"] * 0.2,
    )
    page.mouse.down()
    page.mouse.move(
    ...
    page.mouse.up()
    _wait_until(page, lambda: bool(project["workspace"].get("masks", {}).get("aaaaaaaa")))
    assert _alpha_sum(page) > 0

    page.locator('#historyStrip [data-asset-id="bbbbbbbb"]').click()
    page.wait_for_function("state.selectedId === 'bbbbbbbb'")
    assert _alpha_sum(page) == 0
    page.locator('#historyStrip [data-asset-id="aaaaaaaa"]').click()
    ...
    _wait_until(
        page,
        lambda: project["workspace"].get("settings", {}).get("default_folder")
        == "D:\\ImageLab\\Projects",
    )
    expect(page.locator("#m2aSaveState")).to_have_text("сохранено")
    page.close()


def test_m2a_batch_cancel_preserves_completed_results(browser) -> None:
    page = browser.new_page(viewport={"width": 1280, "height": 900})
    project, process_calls = _load(page)
    page.locator('[data-module="batch"]').click()
    page.locator("#m2aBatchAll").click()
    page.locator("#m2aRunBatch").click()
    _wait_until(page, lambda: len(process_calls) > 0, timeout_ms=10_000)
    page.evaluate("window.imagelabM2A.batch.cancelled = true")
    expect(page.locator("#m2aBatchJob span").first).to_have_text("Отменено", timeout=10_000)
    assert 1 <= len(process_calls) < 3
    reports = project["workspace"].get("batch_reports", [])
    assert any(item.get("status") == "CANCELLED" for report in reports for item in report.get("items", []))
    expect(page.locator('#m2aBatchReport [data-status="PASS"]')).to_have_count(len(process_calls))
    expect(page.locator('#m2aBatchReport [data-status="CANCELLED"]')).to_have_count(1)
    page.close()
