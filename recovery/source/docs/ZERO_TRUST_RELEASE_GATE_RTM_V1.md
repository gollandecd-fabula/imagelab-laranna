...
| ZTR-011 | Preserve complete evidence | ZTR-M6 | Always aggregate pass or fail evidence, including pinned G7 and physical records | Final archive and artifact checks | `release-evidence.zip`, verdict artifact | Logs, traces, outputs, hashes, G7, physical record and verdict are preserved | IMPLEMENTED; CURRENT-HEAD REVALIDATION REQUIRED |
...
| ZTR-013 | Never publish on partial evidence | ZTR-M6 | Clear stale normal/Genesis outputs before evaluation and on failure; Genesis emits only Genesis-specific status/name/record | Positive and adversarial tests | workflow, verdict, authorization record | Any missing gate, invalid history/G7/L5, identity mismatch or stale/ordinary Genesis output blocks authorization | CORRECTION PUSHED; CI AND INDEPENDENT REVIEW PENDING |
