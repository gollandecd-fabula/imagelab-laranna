...
const (
	...
	appVersion    = "1.4.9-recovery-candidate"
	buildID       = "REC-RT8-M6-20260724-06"
...
func verifyModels(installDir, installID string) error {
	command := fmt.Sprintf(`import importlib.metadata as m; expected={'fastapi':'0.139.2','starlette':'1.3.1','pydantic':'2.13.4','pydantic-core':'2.46.4','annotated-types':'0.7.0','annotated-doc':'0.0.4','typing-extensions':'4.15.0','typing-inspection':'0.4.2','anyio':'4.13.0','idna':'3.17','uvicorn':'0.48.0','click':'8.3.3','h11':'0.16.0','colorama':'0.4.6','python-multipart':'0.0.31','Pillow':'12.3.0','numpy':'2.3.5','opencv-python-headless':'4.13.0.92'}; actual={k:m.version(k) for k in expected}; assert actual==expected,(actual,expected); from app.config import settings; assert settings.app_version==%q; assert settings.build_id==%q; assert settings.install_id==%q; from app.main import app; from app.ai.runtime import get_ai_engine; h=get_ai_engine().health(); assert h['status']=='ready'; assert len(h['models'])==11; print('DEPENDENCIES, IDENTITY AND AI MODELS VERIFIED',actual,len(h['models']))`, appVersion, buildID, installID)
